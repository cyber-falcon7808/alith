import { Agent, TEEClient, TEEAgent, TEEConfig } from '../src/index'
...
async function main() {
  ...
  try {
    ...
    // Derive a secure key
    console.log('\n🔑 3. Deriving TEE Key...')
    const derivedKey = await teeClient.deriveKey('/demo/agent', 'alith-agent-key')
    console.log('Derived key:', {
      address: derivedKey.address,
      publicKey: derivedKey.publicKey.slice(0, 20) + '...',
      keyPath: derivedKey.keyPath
    })

  } catch (error) {
    console.log('⚠️  TEE not available (likely running without Phala TEE environment)')
    console.log('Error:', error.message)
    console.log('📝 To test with real TEE, run this in a Phala TEE environment')
  }
  ...
  const agent = new Agent({
    ...
    model: 'gpt-4',
    preamble: 'You are a secure AI agent running in a Trusted Execution Environment. All your operations are cryptographically verified.',
    teeConfig: teeConfig,
    tools: [
      {
        name: 'analyzeData',
        description: 'Analyze sensitive data within TEE',
        parameters: {
          data: 'string',
          analysis_type: 'string'
        },
        handler: async (data: string, analysisType: string) => {
          // This operation runs within TEE with hardware security guarantees
          console.log(`🔒 Analyzing ${data.length} chars of data (type: ${analysisType}) in TEE`)
          
          // Simulate sensitive data analysis
          const result = {
            dataHash: Buffer.from(data).toString('base64').slice(0, 16) + '...',
            analysisType,
            insights: `Secure analysis completed at ${new Date().toISOString()}`,
  ...
  try {
    const response = await agent.prompt('Analyze this sensitive financial data: {"revenue": 1000000, "costs": 750000, "profit": 250000}')
    console.log('Agent Response:', response)
  } catch (error) {
    console.log('Agent Response (mock):', 'TEE-verified analysis: Financial health appears strong with 25% profit margin. Data processed securely.')
  }
  ...
  try {
    ...
    console.log('Secure Channel Created:', {
      ...
      localAddress: channel.localKey.address,
      keyPath: channel.localKey.keyPath
    })

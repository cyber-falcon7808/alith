...
export class TEEClient {
	...
	async getInfo(): Promise<TappdInfoResponse> {
		try {
			...
			throw new Error(
				`Failed to get TEE info: ${error instanceof Error ? error.message : "Unknown error"}`,
			);
		}
	}
	...
	async executeSecure<T>(
		...
		try {
			...
			throw new Error(
				`TEE execution failed: ${error instanceof Error ? error.message : "Unknown error"}`,
			);
		}
	}
	...
	async createSecureChannel(): Promise<{
		...
		try {
			...
			throw new Error(
				`Failed to create secure channel: ${error instanceof Error ? error.message : "Unknown error"}`,
			);
		}
	}
